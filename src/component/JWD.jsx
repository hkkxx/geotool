...
export default function JWD() {
    ...
    return (
        <div className={" grid grid-rows-3 justify-items-center  gap-1 m-auto "}>
            <div className={clsx("w-72 row-span-1 transition duration-700", {
                "row-start-3": order, "row-start-1": !order})}>
            ...
            <div className={" space-x-2  text-center row-span-1  row-start-2"}>
                <span className={""}><Button type="primary" size={"middle"}
                                             onClick={() => setOrder(!order)}>交换位置</Button></span>
                <span className={""}><Button size={"middle"} onClick={convert}>转换</Button></span>
            </div>

            <div className={clsx("w-60 row-span-1 row-start-1 ", {"row-start-1 ": order, "row-start-3": !order})}>
